...
public class PostDaoImpl implements PostDao {
    ...
    public void saveImage(Long postId, byte[] imageData, String contentType) {
        ...
        jdbcTemplate.update("INSERT INTO post_images (post_id, image_data, content_type) VALUES (?, ?, ?)", postId, imageData, contentType);
    }

    @Override  
    public Optional<byte[]> getImage(Long postId) {
        try {
            return Optional.ofNullable(jdbcTemplate.queryForObject("SELECT image_data FROM post_images WHERE post_id = ?", byte[].class, postId));
        } catch (Exception e) {
            return Optional.empty();
        }
    }
    ...
    public Optional<String> getImageContentType(Long postId) {
        try {
            return Optional.ofNullable(jdbcTemplate.queryForObject("SELECT content_type FROM post_images WHERE post_id = ?", String.class, postId));
        } catch (Exception e) {
            return Optional.empty();
        }
    }
    ...
    private void saveTags(Long postId, List<String> tagNames) {
        for (String tagName : tagNames) {
            ...
            Long tagId = existingTag.map(com.myblog.model.Tag::getId)
                                    .orElseGet(() -> tagDao.create(cleanTagName).getId());
